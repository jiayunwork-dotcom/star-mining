...
function IntelPanel() {
  ...
  const getAvailableMissions = (spyLevel) => {
    const missions = [
    ...
    return missions.filter((m) => levelOrder[spyLevel] >= levelOrder[m.minLevel]);
  };
        ...
        <div className="diplomacy-section">
          ...
          {mySpies.length === 0 ? (
            ...
            <div className="spy-list">
              {mySpies.map((spy) => (
                <div key={spy.id} className="spy-item" style={{ padding: '8px', marginBottom: '6px', border: '1px solid #3a3a6a', borderRadius: '4px', background: spy.status === 'on_mission' ? 'rgba(68,136,255,0.1)' : 'rgba(255,255,255,0.03)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      <strong>{SPY_LEVEL_NAMES[spy.level] || spy.level}</strong>
                      {spy.is_double_agent && (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <select value={selectedSpy} onChange={(e) => { setSelectedSpy(e.target.value); setMissionType(''); }} className="diplomacy-select">
                  ...
                  {idleSpies.map((s) => (
                    <option key={s.id} value={s.id}>
                      {SPY_LEVEL_NAMES[s.level]} - 暴露值{s.exposure} ({s.id.slice(0, 12)})
                    </option>
                  ))}
                </select>
                ...
                {selectedSpy && (
                  <select value={missionType} onChange={(e) => setMissionType(e.target.value)} className="diplomacy-select">
                    ...
                    {getAvailableMissions(mySpies.find((s) => s.id === selectedSpy)?.level || 'junior').map((m) => (
                      <option key={m.type} value={m.type}>
                        {m.name} - {SPY_MISSION_DESCRIPTIONS[m.type]}
                      </option>
                    ))}
                  </select>
                )}
                ...
                return (
                  <div key={intel.id} className="intel-item" style={{ padding: '8px', marginBottom: '6px', border: '1px solid #3a3a6a', borderRadius: '4px', background: turnsLeft <= 1 ? 'rgba(255,68,68,0.1)' : 'rgba(255,255,255,0.03)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      ...
                      <span style={{ fontSize: '11px', color: turnsLeft <= 1 ? '#FF4444' : '#888' }}>
                        剩余{turnsLeft}回合
                      </span>
                    </div>
                    <div style={{ fontSize: '12px', color: '#AAA', marginTop: '4px' }}>
        ...
        <div className="diplomacy-section">
          ...
          {myListings.length > 0 && (
            <div style={{ marginBottom: '12px' }}>
              <h5>我的挂单</h5>
              {myListings.map((l) => (
                <div key={l.id} style={{ padding: '6px 8px', marginBottom: '4px', border: '1px solid #FFCC44', borderRadius: '4px', fontSize: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span>价格: {Math.round(l.price)}¢ | 剩余{l.remaining_turns}回合</span>
